...
TreeNode *buildHuffmanTree(vector<pair<unsigned char, int>> freq_map) {
    priority_queue<TreeNode *, vector<TreeNode *>, TreeComparator> huff_queue;
    for (int i = 0; i < freq_map.size(); i++) {
        TreeNode *node = new TreeNode();
        node->frequency = freq_map[i].second;
        node->character = freq_map[i].first;
...
string toBinary(unsigned char a) {
    ...
    while (a != 0) {
        string bit = (a % 2) ? "1" : "0";
        outputBuffer = bit + outputBuffer;
        a /= 2;
    }
    if (outputBuffer.size() < 8) {
        int deficit = 8 - outputBuffer.size();
...
void traverseHuffmanTree(TreeNode *root, string prev, string toAppend, map<unsigned char, string> &codemap) {
    prev += toAppend;
...
unsigned char *readFileIntoBuffer(const char *path, int &sz) {
    ...
    fseek(fp, 0, SEEK_END) moves the pointer to the end.
...
void compressFile(const char *path, const char *output_path, map<unsigned char, string> &codes) {
    ...
    TreeNode *root = buildHuffmanTree(convertToVector(freqtable));
